...
public class ResBookDTO {
    ...
    private List<ResBookImageDTO> images;
    ...
    public static ResBookDTO from(Book book) {
        ...
        List<ResBookImageDTO> imageDtos = new ArrayList<>();
        if (book.getBookImages() != null && !book.getBookImages().isEmpty()) {
            for (BookImage bi : book.getBookImages()) {
                imageDtos.add(ResBookImageDTO.from(bi));
            }
            BookImage primary = book.getBookImages().stream().filter(BookImage::isPrimaryImage).findFirst()
                    .orElse(book.getBookImages().getFirst());
            coverImage = primary.getRelativePath();
        }
        resBookDTO.setImages(imageDtos);
        resBookDTO.setImage(coverImage);
